...
class CalendarWidget extends StatefulWidget {
  const CalendarWidget({super.key, required this.widgetCount, required this.widgetNum, required this.widgetWidth, required this.widgetCountGetter});

  final int widgetCount; //0: 단일 조회, 1: 궁합 조회
  final int widgetNum;
  final double widgetWidth;
  final widgetCountGetter;
...
class _CalendarWidget extends State<CalendarWidget> {
  ...
  Widget build(BuildContext context) {
    ...
    if(widget.widgetCountGetter() == 1){
      widgetWidth = 1200;
    } else if(widget.widgetCountGetter() == 2){
      widgetWidth = 600;
    } else {
      widgetWidth = 400;
    }
